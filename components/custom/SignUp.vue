            ...
            <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
                <div class="px-5 py-7">
                    <form @submit.prevent="register">
                        ...
                        <input v-model="signUpData.name" type="text" required class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">E-mail</label>
                        <input v-model="signUpData.email" type="email" required class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Password</label>
                        <input v-model="signUpData.password" type="password" required class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
...
<script>
    ...
    export default {
        ...
        methods: {
            register(){
                axios.post("http://localhost:4000/api/auth/register",this.signUpData)
                    .then(response => {
        ...
        mounted() {
            this.$parent.$parent.hideHeaderFooter();
        }
    }
</script>
